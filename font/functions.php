...
function mesmerize_enqueue_styles($textDomain, $ver, $is_child)
{
    ...
    if (apply_filters('mesmerize_load_bundled_version', true)) {
        ...
        mesmerize_enqueue_style(
            'mdb',
            array(
                'src'     => get_template_directory_uri() . '/assets/css/mdb.css',
                'has_min' => true,
            )
        );
...
function mesmerize_enqueue_scripts($textDomain, $ver, $is_child)
{
    ...
    if (apply_filters('mesmerize_load_bundled_version', true)) {
        ...
        mesmerize_enqueue_script(
            $textDomain . 'Jquery',
            array(
                'src'     => get_template_directory_uri() . '/assets/js/jquery-3.4.1.min.js',
                'deps'    => array('jquery'),
                'has_min' => true,
            )
        );
        mesmerize_enqueue_script(
            ...
            array(
                'src'     => get_template_directory_uri() . '/assets/js/bootstrap.js',
                'deps'    => array('jquery'),
                'has_min' => true,
            )
        );
        mesmerize_enqueue_script(
            $textDomain . 'mdb',
            array(
                'src'     => get_template_directory_uri() . '/assets/js/mdb.js',
                'deps'    => array('jquery'),
                'has_min' => true,
            )
        );
        mesmerize_enqueue_script(
            $textDomain . 'popper',
            array(
                'src'     => get_template_directory_uri() . '/assets/js/popper.min.js',
                'deps'    => array('jquery'),
                'has_min' => true,
            )
        );
